...
void InterpretManager::Where_Clause(int pos, vector<where>& w, Table* tb)
{
	...
	while (query[i] != ";")
	{
		...
		int attr_type;//the data type of the attribute
					  /*for (j = 0; j < Column_Size; j++)//check attribute name procedure
					  {
					  if (query[i] == Table_Attr.name[j])
					  {
					  attr_type = Table_Attr.flag[j];
					  Column_Where.push_back(j);
					  break;
					  }
