...
static int Name_Length(char* begin, int pos)//calculate the length of a string
{
    ...
    //cout << "len method: " << len << "  minus method: " << pos - former + 1 << endl;
    return len + 1;
}
...
void CatalogManager::Mem_Move_Attribute(vector<Attribute>& attr, char* Block, int& pos, int RW)
{
    char temp[20];
    int Step;
    for (int i = 0; i < attr.size(); i++) //move attribute name
    {
        if (RW == 0)
        {
    ...
    for (int i = 0; i < attr.size(); i++) //type
    {
        if (RW == 0)
        {
    ...
    for (int i = 0; i < attr.size(); i++) //uniqueness
    {
        if (RW == 0)
        {
...
void CatalogManager::Mem_Move_Index(vector<Index>& ind, char* Block, int& pos, int RW)
{
    char temp[20];
    int Step;
    for (int i = 0; i < ind.size(); i++) //index name
    {
        if (RW == 0)
        {
            Step = Name_Length(Block, pos);
    ...
    for (int i = 0; i < ind.size(); i++) //index location
    {
        if (RW == 0)
        {
